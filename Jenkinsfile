podTemplate(label: 'catalog-service-builder', containers: [
    ...
    containerTemplate(name: 'stock-service-chart', image: 'enxebre/stock-service-chart:v1', alwaysPullImage: true, ttyEnabled: true, command: 'helm serve --address 0.0.0.0:8878 --repo-path /usr/stock-service-chart/charts/stock'),
  ...
  volumes: [hostPathVolume(mountPath: '/var/run/docker.sock', hostPath: '/var/run/docker.sock')]) {
    ...
    node ('catalog-service-builder') {
        ...
        container('docker') {
            stage 'Build Service'
            sh 'docker build -t enxebre/catalog:v1 .'

            stage 'Push Service'
            sh "docker login -u enxebre -p ${dockerHubPass}"
            sh 'docker push enxebre/catalog:v1'
        }
